...
import { AlertTriangle, ArrowDownRight, ArrowUpRight, Check, ChevronRight, Copy, MoreHorizontal, Plus, RefreshCw, Shield, Sparkles, UserPlus, } from "lucide-react";
...
function toneClasses(tone) {
    switch (tone) {
        case "primary":   return "bg-primary/12 text-primary border-primary/25";
        case "secondary": return "bg-secondary/12 text-secondary border-secondary/25";
        case "success":   return "bg-success/12 text-success border-success/25";
        case "warning":   return "bg-warning/12 text-warning border-warning/25";
        case "danger":    return "bg-destructive/12 text-destructive border-destructive/25";
    }
}
...
// ─── MetricCards ────────────────────────────────────────────────────────────
// lg:grid-cols-4 activates at 1024px (was xl = 1280px)
    ...
    return (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {cards.map((card, index) => {
                ...
                return (
                    <motion.div key={card.title} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05, duration: 0.25 }}>
                        <Card className="glass-panel mesh-border rounded-[28px] border-border/60 bg-card/70">
                            <CardContent className="p-5">
                                <div className="flex items-start justify-between gap-4">
                                    <div>
                                        <p className="text-sm text-muted-foreground">{card.title}</p>
                                        {/* text-3xl on laptop, text-4xl on xl+ */}
                                        <p className="mt-3 text-3xl xl:text-4xl font-semibold text-foreground">{card.value}</p>
...
// ─── OverviewCharts ──────────────────────────────────────────────────────────
// lg:grid-cols-2 at 1024px, xl:grid-cols-3 at 1280px
export function OverviewCharts() {
    ...
    return (
        <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-[1.4fr_1fr_0.9fr]">
            <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70 xl:col-span-1">
                <CardHeader className="flex-row items-start justify-between space-y-0">
                    ...
                    <Select defaultValue="month">
                        <SelectTrigger className="w-[130px] rounded-2xl border-border/60 bg-background/20"><SelectValue /></SelectTrigger>
                        <SelectContent>
                ...
                <CardContent className="pt-2">
                    {/* h-[260px] on laptop, h-[320px] on xl */}
                    <ChartContainer config={chartConfig} className="h-[260px] xl:h-[320px] w-full">
            ...
            <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                ...
                <CardContent className="pt-0">
                    <ChartContainer config={{ Basic: { label: "Basic", color: "var(--color-chart-2)" }, Pro: { label: "Pro", color: "var(--color-chart-1)" }, Enterprise: { label: "Enterprise", color: "var(--color-chart-3)" }, Custom: { label: "Custom", color: "var(--color-chart-4)" } }} className="h-[260px] xl:h-[320px]">
                        <PieChart>
...
// ─── BottomOverview ──────────────────────────────────────────────────────────
// lg:grid-cols-2 at 1024px, full 3-col at xl
export function BottomOverview() {
    return (
        <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-[1.4fr_1fr_0.9fr] items-start">
            <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                <CardHeader className="flex-row items-start justify-between space-y-0">
                    <div>
            ...
            <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                ...
                <CardContent className="grid gap-3 sm:grid-cols-2">
                    {[
                        ...
                        return (
                            <motion.button whileHover={{ y: -3 }} key={action.label} className="glass-panel rounded-3xl border-border/60 p-4 text-left">
...
// ─── KPIFooter ───────────────────────────────────────────────────────────────
// lg:grid-cols-3 bridges the gap between sm:2 and xl:5
export function KPIFooter() {
    return (
        <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
            <CardContent className="grid gap-4 p-0 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
                {kpiStrip.map((item, index) => (
                    <div key={item.label} className={cn("p-5", index < kpiStrip.length - 1 && "border-b border-border/50 xl:border-b-0 xl:border-r")}>
                        ...
                        <p className="mt-3 text-3xl xl:text-4xl font-semibold text-foreground">{item.value}</p>
...
// ─── DataTable ───────────────────────────────────────────────────────────────
// overflow-x-auto prevents table blowout on 13-14" screens
function DataTable({ data, columns }) {
    const table = useReactTable({ data, columns, getCoreRowModel: getCoreRowModel() });
    return (
        <div className="overflow-x-auto overflow-hidden rounded-[24px] border border-border/60 bg-background/10">
            <Table>
                <TableHeader>
...
function TableToolbar({ placeholder, action }) {
    return (
        <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-1 flex-col gap-3 md:flex-row">
                ...
                <Select defaultValue="all">
                    <SelectTrigger className="w-full rounded-2xl border-border/60 bg-background/10 md:w-[160px]"><SelectValue placeholder="All" /></SelectTrigger>
                    <SelectContent>
...
export function UserManagementSection() {
    ...
    const columns = useMemo(() => [
        { id: "select", header: () => <Checkbox aria-label="Select all" />, cell: () => <Checkbox aria-label="Select row" /> },
        {
...
// ─── AIJobsSection ───────────────────────────────────────────────────────────
// stats: lg:grid-cols-3, charts: lg:grid-cols-2
        ...
        <div className="space-y-4">
            ...
            <div className="grid gap-4 lg:grid-cols-2">
                <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                    <CardHeader className="flex-row items-center justify-between space-y-0">
                        ...
                        <Select defaultValue="7d">
                            <SelectTrigger className="w-[140px] rounded-2xl border-border/60 bg-background/10"><SelectValue /></SelectTrigger>
                            <SelectContent>
                    ...
                    <CardContent>
                        <ChartContainer config={{ aiUsage: { label: "Jobs", color: "var(--color-chart-1)" } }} className="h-[240px] xl:h-[300px]">
                ...
                <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                    ...
                    <CardContent>
                        <ChartContainer config={{ Completed: { label: "Completed", color: "var(--color-chart-1)" }, Processing: { label: "Processing", color: "var(--color-chart-3)" }, Failed: { label: "Failed", color: "var(--color-chart-5)" } }} className="h-[240px] xl:h-[300px]">
                            <PieChart>
                                <ChartTooltip content={<ChartTooltipContent hideLabel />} />
                                <Pie data={[{ name: "Completed", value: 21562, fill: "var(--color-chart-1)" }, { name: "Processing", value: 2456, fill: "var(--color-chart-3)" }, { name: "Failed", value: 857, fill: "var(--color-chart-5)" }]} dataKey="value" nameKey="name" innerRadius={70} outerRadius={110}>
...
// ─── AnalyticsSection ────────────────────────────────────────────────────────
// All grids start at lg instead of xl
        ...
        <div className="space-y-4">
            ...
            <div className="grid gap-4 lg:grid-cols-2">
                <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                    ...
                    <CardContent>
                        <ChartContainer config={{ revenue: { label: "Revenue", color: "var(--color-chart-1)" }, refunds: { label: "Refunds", color: "var(--color-chart-5)" } }} className="h-[260px] xl:h-[320px]">
                ...
                <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                    ...
                    <CardContent>
                        <ChartContainer config={{ users: { label: "Users", color: "var(--color-chart-2)" }, subscriptions: { label: "Subscriptions", color: "var(--color-chart-3)" } }} className="h-[260px] xl:h-[320px]">
                            <LineChart data={trendData}>
                                <CartesianGrid vertical={false} strokeDasharray="4 4" />
                                <XAxis dataKey="name" tickLine={false} axisLine={false} />
                                <YAxis tickLine={false} axisLine={false} />
            ...
            <div className="grid gap-4 lg:grid-cols-2">
                <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                    ...
                    <CardContent>
                        <ChartContainer config={{ aiUsage: { label: "AI Usage", color: "var(--color-chart-4)" }, upgrades: { label: "Upgrades", color: "var(--color-chart-1)" }, downgrades: { label: "Downgrades", color: "var(--color-chart-5)" } }} className="h-[260px] xl:h-[320px]">
                            <BarChart data={trendData}>
                                <CartesianGrid vertical={false} strokeDasharray="4 4" />
                                <XAxis dataKey="name" tickLine={false} axisLine={false} />
                                <YAxis tickLine={false} axisLine={false} />
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                {cmsSummary.map((item) => (
                    <Card key={item.label} className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                        <CardContent className="p-5">
                            <p className="text-sm text-muted-foreground">{item.label}</p>
                            <p className="mt-4 text-4xl xl:text-5xl font-semibold text-foreground">{item.value}</p>
            ...
            <div className="grid gap-4 lg:grid-cols-[1.35fr_0.65fr]">
                ...
                <div className="space-y-4">
                    <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                        ...
                        <CardContent>
                            <ChartContainer config={{ Pages: { label: "Pages", color: "var(--color-chart-2)" }, Blogs: { label: "Blogs", color: "var(--color-chart-1)" }, FAQs: { label: "FAQs", color: "var(--color-chart-3)" }, Banners: { label: "Banners", color: "var(--color-chart-4)" } }} className="h-[220px] xl:h-[240px]">
                                <PieChart>
                                    <ChartTooltip content={<ChartTooltipContent hideLabel />} />
                                    <Pie data={[{ name: "Pages", value: 42, fill: "var(--color-chart-2)" }, { name: "Blogs", value: 128, fill: "var(--color-chart-1)" }, { name: "FAQs", value: 64, fill: "var(--color-chart-3)" }, { name: "Banners", value: 24, fill: "var(--color-chart-4)" }]} dataKey="value" nameKey="name" innerRadius={56} outerRadius={88}>
                    ...
                    <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                        ...
                        <CardContent className="space-y-3 pt-0">
                            {["Create New Page", "Create New Blog", "Create New FAQ", "Upload Banner"].map((item) => (
...
export function PaymentsSection() {
    ...
    return (
        <div className="space-y-4">
            {/* Fix 1: grid-cols-2 → lg:grid-cols-5 avoids the uneven 3+2 orphan row */}
                ...
                {[
                    ...
                    <Card key={item.label} className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                        <CardContent className="p-5">
                            <p className="text-sm text-muted-foreground">{item.label}</p>
                            <p className="mt-3 text-3xl xl:text-4xl font-semibold text-foreground">{item.value}</p>
                            {/* Fix 3: negative deltas in red, positive in green */}
                            <p className={cn("mt-3 text-sm", item.delta.startsWith("-") ? "text-destructive" : "text-success")}>
            ...
            {/*
              Fix 2: at lg (2 cols), the 3rd div was orphaned in a half-width row.
              Now: lg → 3rd div spans both cols as a 2-col side-by-side row
                   xl → 3rd div becomes its own column, cards stack vertically
            */}
            <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-[1.3fr_0.9fr_0.8fr]">
                <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                    <CardHeader>
                        <CardTitle>Revenue Overview</CardTitle>
                    ...
                    <CardContent>
                        <ChartContainer config={{ revenue: { label: "Revenue", color: "var(--color-chart-1)" } }} className="h-[260px] xl:h-[320px]">
                            <LineChart data={trendData}>
                                <CartesianGrid vertical={false} strokeDasharray="4 4" />
                                <XAxis dataKey="name" tickLine={false} axisLine={false} />
                                <YAxis tickLine={false} axisLine={false} />
                ...
                <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                    ...
                    <CardContent>
                        <ChartContainer config={{ Stripe: { label: "Stripe", color: "var(--color-chart-1)" }, PayPal: { label: "PayPal", color: "var(--color-chart-2)" }, Razorpay: { label: "Razorpay", color: "var(--color-chart-3)" }, Other: { label: "Other", color: "var(--color-chart-4)" } }} className="h-[260px] xl:h-[320px]">
                            <PieChart>
                                <ChartTooltip content={<ChartTooltipContent hideLabel />} />
                                <Pie data={paymentMethodBreakdown} dataKey="value" nameKey="name" innerRadius={68} outerRadius={104}>
                                    {paymentMethodBreakdown.map((entry) => (<Cell key={entry.name} fill={entry.fill} />))}
                                </Pie>
                                <Legend content={<ChartLegendContent />} />
                            </PieChart>
                        </ChartContainer>
                ...
                {/* lg: span both cols as a 2-col horizontal row | xl: single column, stacked */}
                <div className="lg:col-span-2 lg:grid lg:grid-cols-2 lg:gap-4 xl:col-span-1 xl:block xl:space-y-4">
                    <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                        <CardHeader><CardTitle>Gateway Status</CardTitle></CardHeader>
                        <CardContent className="space-y-3 pt-0">
                            {[["Stripe", "99.9%"], ["PayPal", "99.5%"], ["Razorpay", "99.7%"]].map(([name, uptime]) => (
                    ...
                    <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70 xl:mt-0">
                        <CardHeader><CardTitle>Quick Actions</CardTitle></CardHeader>
                        <CardContent className="space-y-3 pt-0">
                            {["Refund Payment", "View Disputes", "Download Invoices", "Gateway Settings"].map((item) => (
                                <Button key={item} variant="outline" className="w-full justify-start rounded-2xl border-border/60 bg-background/10">{item}</Button>
                            ))}
                        </CardContent>
                    </Card>
...
export function SubscriptionSection() {
    ...
    const plans = (Array.isArray(planData) ? planData : planData?.items ?? planCards).map((plan, index) => ({
        ...planCards[index], ...plan,
        price: typeof plan.price === "number" ? `₹${plan.price.toLocaleString()}` : plan.price,
        subscribers: plan.subscribers ?? 0,
    }));
    const tableData = (Array.isArray(subscriptionData) ? subscriptionData : subscriptionData?.items ?? subscriptions).map((s) => ({
        ...s,
        subscriber:  s.subscriber  ?? s.user?.name  ?? "Subscriber",
        email:       s.email       ?? s.user?.email ?? "",
        plan:        s.plan?.name  ?? s.plan         ?? "Plan",
        renewalDate: s.renewalDate ?? s.currentPeriodEnd ?? "-",
        amount:      s.amount      ?? (s.plan?.price ? `₹${s.plan.price}` : "-"),
    }));
    const columns = useMemo(() => [
        {
        ...
        <div className="space-y-4">
            <div className="grid gap-4 lg:grid-cols-2">
                ...
                <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                    ...
                    <CardContent className="space-y-4 pt-0">
                        <ChartContainer config={{ subscriptions: { label: "Growth", color: "var(--color-chart-1)" } }} className="h-[160px] xl:h-[180px]">
                            <LineChart data={trendData}>
                                <CartesianGrid vertical={false} strokeDasharray="4 4" />
                                <XAxis dataKey="name" tickLine={false} axisLine={false} />
                                <YAxis tickLine={false} axisLine={false} />
                        ...
                        <ChartContainer config={{ Basic: { label: "Basic", color: "var(--color-chart-2)" }, Pro: { label: "Pro", color: "var(--color-chart-1)" }, Enterprise: { label: "Enterprise", color: "var(--color-chart-3)" } }} className="h-[200px] xl:h-[220px]">
                            <PieChart>
                                <ChartTooltip content={<ChartTooltipContent hideLabel />} />
                                <Pie data={subscriptionBreakdown.slice(0, 3)} dataKey="value" nameKey="name" innerRadius={56} outerRadius={86}>
                                    {subscriptionBreakdown.slice(0, 3).map((entry) => (<Cell key={entry.name} fill={entry.fill} />))}
                                </Pie>
                                <Legend content={<ChartLegendContent />} />
                            </PieChart>
                        </ChartContainer>
                        <div className="grid gap-3 sm:grid-cols-2">
                            {[["MRR", "$84,240"], ["ARR", "$1,010,880"], ["LTV", "$2,890"], ["ARPU", "$24.01"]].map(([label, value]) => (
    ...
    return (
        <div className="grid gap-4 lg:grid-cols-[1.35fr_0.8fr]">
                ...
                <CardContent>
                    ...
                    <div className="space-y-4">
                        {notificationRows.map((item) => (
                            <button key={item.title + item.time} onClick={() => setSelected(item)} className={cn("flex w-full items-start gap-4 rounded-[24px] border p-4 text-left transition-all", selected.title === item.title ? "border-primary/40 bg-primary/8 shadow-[var(--shadow-glow)]" : "border-border/60 bg-background/10 hover:bg-accent/40")}>
            ...
            <div className="space-y-4">
                ...
                <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                    <CardHeader><CardTitle>Notification Overview</CardTitle></CardHeader>
                    <CardContent className="grid gap-3 sm:grid-cols-2 pt-0">
                        {[["Total", "128", "+12.5%"], ["Unread", "8", "+33.3%"], ["Push", "75", "+8.3%"], ["Email", "53", "+16.2%"]].map(([label, value, delta]) => (
                            <div key={label} className="rounded-2xl border border-border/60 bg-background/10 p-4">
                                <p className="text-sm text-muted-foreground">{label}</p>
                                <p className="mt-3 text-3xl xl:text-4xl font-semibold text-foreground">{value}</p>
...
export function ApiKeysSection() {
    const form = useForm({ resolver: zodResolver(apiKeySchema), defaultValues: { name: "", environment: "Production" } });
    const onSubmit = form.handleSubmit(() => { form.reset({ name: "", environment: "Production" }); });
    const columns = useMemo(() => [
    ...
    return (
        <div className="grid gap-4 lg:grid-cols-2">
            <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                ...
                <CardContent><DataTable data={apiKeys} columns={columns} /></CardContent>
            </Card>
            <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                <CardHeader>
                        ...
                        <form onSubmit={onSubmit} className="space-y-4">
                            <FormField control={form.control} name="name" render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl><Input {...field} placeholder="Production API" className="rounded-2xl border-border/60 bg-background/10" /></FormControl>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
                <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                    ...
                    <CardContent className="space-y-4 pt-0">
                        <div className="rounded-[28px] border border-border/60 bg-background/10 p-5">
                            <div className="flex items-end justify-between gap-4">
                                <div>
                                    ...
                                    <p className="mt-3 text-4xl xl:text-5xl font-semibold text-foreground">1.25M</p>
                                </div>
                        ...
                        <ChartContainer config={{ aiUsage: { label: "Usage", color: "var(--color-chart-1)" } }} className="h-[220px] xl:h-[260px]">
                            <BarChart data={trendData}>
                                <CartesianGrid vertical={false} strokeDasharray="4 4" />
                                <XAxis dataKey="name" tickLine={false} axisLine={false} />
                                <YAxis tickLine={false} axisLine={false} />
                ...
                <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                    ...
                    <CardContent><DataTable data={creditPurchases} columns={columns} /></CardContent>
                </Card>
            </div>
        </div>
    );
    ...
    return (
        <div className="grid gap-4 lg:grid-cols-2">
            ...
            <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                ...
                <CardContent><DataTable data={securityLogs} columns={columns} /></CardContent>
            </Card>
        </div>
    );
}
            ...
            <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                <CardHeader>
                    ...
                    <Tabs defaultValue="General">
                        <TabsList className="rounded-2xl bg-background/15 p-1">
                            {settingsTabs.map((tab) => (<TabsTrigger key={tab} value={tab} className="rounded-xl">{tab}</TabsTrigger>))}
                        </TabsList>
                        {settingsTabs.map((tab) => (
                            <TabsContent key={tab} value={tab} className="mt-6">
                                <div className="space-y-4">
                                    <div className="grid gap-4 md:grid-cols-2">
                                        {[["Platform Name", "SmartLogix AI"], ["Company Name", "SmartLogix Technologies Inc."], ["Support Email", "support@smartlogix.ai"], ["Support Phone", "+1 (555) 123-4567"]].map(([label, value]) => (
    ...
    return (
        <div className="space-y-4">
            ...
            <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                ...
                <CardContent>
                    <TableToolbar placeholder="Search tickets by ID, user or subject..." action={<Button variant="outline" className="rounded-2xl border-border/60 bg-background/10">Filters</Button>} />
                    <DataTable
                        data={[
                        ...
                        columns={[
                            ...
                            { accessorKey: "subject", header: "Subject",  cell: ({ row }) => <p className="font-medium text-foreground">{row.original.subject}</p> },
                            { accessorKey: "user",    header: "User" },
                            {
        ...
        <div className="space-y-4">
            ...
            <div className="grid gap-4 lg:grid-cols-[1.4fr_0.6fr]">
                <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                    ...
                    <CardContent>
                        <TableToolbar placeholder="Search pages..." action={<Button variant="outline" className="rounded-2xl border-border/60 bg-background/10">Filters</Button>} />
                        <DataTable
                            data={[
                ...
                <div className="space-y-4">
                    <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
                        ...
                        <CardContent className="space-y-3 pt-0">
                            {[["SSL Certificate", "Valid"], ["Uptime", "Operational"], ["CDN", "Operational"], ["DNS", "Operational"], ["Sitemap", "Operational"]].map(([label, status]) => (
