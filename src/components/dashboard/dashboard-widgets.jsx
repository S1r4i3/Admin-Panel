    ...
    return (<div className="grid gap-4 xl:grid-cols-[1.4fr_1fr_0.9fr] items-start">
      ...
      <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
        ...
        <CardContent className="grid gap-3 sm:grid-cols-2">
          {[
            ...
            return (<motion.button whileHover={{ y: -3 }} key={action.label} className="glass-panel rounded-3xl border-border/60 p-4 text-left">
      ...
      <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
        ...
        <CardContent className="space-y-4 pt-0">
          {healthChecks.slice(0, 5).map((check) => (<div key={check.label} className="flex items-center justify-between gap-3 border-b border-border/50 pb-3 last:border-b-0 last:pb-0">
...
function DataTable({ data, columns, }) {
    ...
    return (<div className="overflow-hidden rounded-[24px] border border-border/60 bg-background/10">
      <Table>
        <TableHeader>
          {table.getHeaderGroups().map((headerGroup) => (<TableRow key={headerGroup.id} className="border-border/60 hover:bg-transparent">
              {headerGroup.headers.map((header) => (<TableHead key={header.id} className="h-12 px-4 text-xs uppercase tracking-[0.12em] text-muted-foreground">
                  {header.isPlaceholder ? null : flexRender(header.column.columnDef.header, header.getContext())}
                </TableHead>))}
            </TableRow>))}
        </TableHeader>
        <TableBody>
          {table.getRowModel().rows.map((row) => (<TableRow key={row.id} className="border-border/50 hover:bg-accent/40">
              {row.getVisibleCells().map((cell) => (<TableCell key={cell.id} className="px-4 py-4 align-middle">
...
function TableToolbar({ placeholder, action, }) {
    return (<div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
...
export function AIJobsSection() {
    ...
    return (<div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        ].map((item) => (<Card key={item.label} className="glass-panel rounded-[28px] border-border/60 bg-card/70">
            <CardContent className="p-5">
              <p className="text-sm text-muted-foreground">{item.label}</p>
              <p className="mt-3 text-4xl font-semibold text-foreground">{item.value}</p>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.25fr_0.95fr]">
        ...
        ]} dataKey="value" nameKey="name" innerRadius={70} outerRadius={110}>
...
export function AnalyticsSection() {
    return (<div className="space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        ].map(([label, value]) => (<div key={label} className="glass-panel rounded-2xl px-4 py-3">
...
export function CMSSection() {
    ...
    return (<div className="space-y-4">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {cmsSummary.map((item) => (<Card key={item.label} className="glass-panel rounded-[28px] border-border/60 bg-card/70">
            <CardContent className="p-5">
              <p className="text-sm text-muted-foreground">{item.label}</p>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.35fr_0.65fr]">
        ...
        <div className="space-y-4">
        ...
        ]} dataKey="value" nameKey="name" innerRadius={56} outerRadius={88}>
          ...
          <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
            ...
            "Create New FAQ",
            "Upload Banner",
        ].map((item) => (<Button key={item} variant="outline" className="w-full justify-start rounded-2xl border-border/60 bg-background/10">{item}</Button>))}
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.3fr_0.9fr_0.8fr]">
        ...
        <div className="space-y-4">
          <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
            ...
            ["PayPal", "99.5%"],
            ["Razorpay", "99.7%"],
        ].map(([name, uptime]) => (<div key={name} className="flex items-center justify-between rounded-2xl border border-border/60 bg-background/10 px-4 py-3">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.45fr_0.85fr]">
          ...
          <CardContent className="grid gap-4 lg:grid-cols-3">
            {plans.map((plan) => (<div key={plan.name} className={cn("glass-panel relative rounded-[28px] p-5", plan.featured && "shadow-[var(--shadow-glow)]")}>
                ...
                <div className="space-y-3">
                  {plan.features.map((feature) => (<div key={feature} className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
          ...
          <CardContent className="space-y-4 pt-0">
            ...
            ["LTV", "$2,890"],
            ["ARPU", "$24.01"],
        ].map(([label, value]) => (<div key={label} className="rounded-2xl border border-border/60 bg-background/10 p-4">
      ...
      <div className="space-y-4">
        ...
        <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
          ...
          <CardContent className="grid gap-3 sm:grid-cols-2 pt-0">
            ...
            ["Push", "75", "+8.3%"],
            ["Email", "53", "+16.2%"],
        ].map(([label, value, delta]) => (<div key={label} className="rounded-2xl border border-border/60 bg-background/10 p-4">
                <p className="text-sm text-muted-foreground">{label}</p>
        ...
        <CardContent>
          <Form {...form}>
            <form onSubmit={onSubmit} className="space-y-4">
              ...
              <FormField control={form.control} name="environment" render={({ field }) => (<FormItem>
...
export function SecuritySection() {
    ...
    return (<div className="grid gap-4 xl:grid-cols-[0.95fr_1.05fr]">
      <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
        <CardHeader>
        ...
        ].map(([label, detail, checked]) => (<div key={String(label)} className="flex items-center justify-between gap-4 rounded-3xl border border-border/60 bg-background/10 p-4">
              <div>
    ...
    return (<div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
      <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
        <CardHeader>
          ...
          <Tabs defaultValue="General">
            ...
            {settingsTabs.map((tab) => (<TabsContent key={tab} value={tab} className="mt-6">
                <div className="space-y-4">
      ...
      <div className="space-y-4">
        <Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
          ...
          <CardContent className="space-y-3 pt-0">
            {healthChecks.map((check) => (<div key={check.label} className="flex items-center justify-between rounded-2xl border border-border/60 bg-background/10 px-4 py-3">
                <span className="text-sm text-foreground">{check.label}</span>
                <Badge className="rounded-full bg-success/12 text-success">{check.status}</Badge>
              </div>))}
          </CardContent>
        ...
        ].map(([text, time]) => (<div key={String(text)} className="flex items-center justify-between gap-3 rounded-2xl border border-border/60 bg-background/10 px-4 py-3">
...
export function PlaceholderSection({ title, description, }) {
    return (<Card className="glass-panel rounded-[28px] border-border/60 bg-card/70">
      ...
      <CardContent className="grid gap-4 md:grid-cols-3">
        {Array.from({ length: 6 }).map((_, index) => (<div key={index} className="rounded-[24px] border border-border/60 bg-background/10 p-4">
