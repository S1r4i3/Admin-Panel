...
function SmartLogixMark() {
  return (
    <div className="relative h-11 w-11 shrink-0 rounded-2xl mesh-border brand-gradient p-[1px] shadow-[var(--shadow-glow)]">
...
function SidebarContent({ mobile = false }: { mobile?: boolean }) {
  ...
  return (
    <div
      className={cn(
        "glass-panel mesh-border flex h-full flex-col overflow-hidden rounded-[28px]",
        mobile ? "w-full rounded-none border-0 bg-sidebar/95" : "",
      )}
    >
      <div className="flex items-center gap-3 border-b border-border/60 px-5 py-5">
        ...
        {!mobile && (
          <Button
            variant="ghost"
            size="icon"
            className="ml-auto rounded-full border border-border/60 bg-card/40"
                          ...
                          >
                            ...
                            {(!sidebarCollapsed || mobile) && (
                              <>
                                ...
                                {item.badge ? (
                                  <Badge className={cn(
                                    "relative rounded-full px-2 py-0 text-[11px]",
                                    active ? "bg-white/15 text-primary-foreground" : "bg-primary/15 text-primary",
                                  )}>
                                    {item.badge}
                                  </Badge>
                                ) : null}
...
function TopBar({ title, subtitle, actions }: Pick<AppShellProps, "title" | "subtitle" | "actions">) {
