...
function SmartLogixMark() {
    return (<div className="relative h-11 w-11 shrink-0 rounded-2xl mesh-border brand-gradient p-[1px] shadow-[var(--shadow-glow)]">
      <div className="glass-panel flex h-full w-full items-center justify-center rounded-[calc(var(--radius)-2px)]">
        <div className="relative h-6 w-6 rotate-45">
          <span className="absolute left-0 top-0 h-3 w-3 rounded-[4px] bg-primary/90"/>
          <span className="absolute right-0 top-0 h-3 w-3 rounded-[4px] bg-secondary/90"/>
          <span className="absolute bottom-0 left-0 h-3 w-3 rounded-[4px] bg-chart-3/90"/>
          <span className="absolute bottom-0 right-0 h-3 w-3 rounded-[4px] bg-primary/35"/>
        </div>
      </div>
    </div>);
}
function SidebarContent({ mobile = false }) {
    ...
    return (<div className={cn("glass-panel mesh-border flex h-full flex-col overflow-hidden rounded-[28px]", mobile ? "w-full rounded-none border-0 bg-sidebar/95" : "")}>
      <div className="flex items-center gap-3 border-b border-border/60 px-5 py-5">
        <SmartLogixMark />
        {(!sidebarCollapsed || mobile) && (<div className="min-w-0">
            <p className="font-display text-2xl font-bold text-foreground">SmartLogix <span className="text-gradient">AI</span></p>
            <p className="text-sm text-muted-foreground">Enterprise Control Center</p>
          </div>)}
        {!mobile && (<Button variant="ghost" size="icon" className="ml-auto rounded-full border border-border/60 bg-card/40" onClick={toggleSidebar}>
            <ChevronLeft className={cn("transition-transform", sidebarCollapsed && "rotate-180")}/>
          </Button>)}
      </div>

      {(!sidebarCollapsed || mobile) && (<div className="px-5 pt-5">
      ...
      <div className="mt-4 flex-1 overflow-y-auto px-3 pb-4">
        {sections.map((section) => (<div key={section} className="mb-5">
            {(!sidebarCollapsed || mobile) && (<p className="px-3 pb-2 text-xs uppercase tracking-[0.18em] text-muted-foreground/80">
                {section}
              </p>)}
            <div className="space-y-1.5">
              {navItems
                ...
                return (<TooltipProvider key={item.href} delayDuration={150}>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Link to={item.href} className={cn("group relative flex items-center gap-3 overflow-hidden rounded-2xl border px-3 py-3 transition-all duration-300", sidebarCollapsed && !mobile ? "justify-center px-2.5" : "", active
                        ...
                        : "border-transparent text-sidebar-foreground hover:border-border/60 hover:bg-sidebar-accent/70 hover:shadow-[var(--shadow-soft)]")}>
                            {active && (<motion.span layoutId="active-nav-pill" className="absolute inset-0 rounded-2xl bg-transparent" transition={{ type: "spring", stiffness: 280, damping: 28 }}/>)}
                            <span className={cn("relative flex h-10 w-10 items-center justify-center rounded-2xl border", active
                        ...
                        : "border-border/60 bg-card/30 group-hover:border-primary/40 group-hover:bg-primary/10")}>
                            ...
                            {(!sidebarCollapsed || mobile) && (<>
                                ...
                                {item.badge ? (<Badge className={cn("relative rounded-full px-2 py-0 text-[11px]", active ? "bg-white/15 text-primary-foreground" : "bg-primary/15 text-primary")}>
                                    {item.badge}
                                  </Badge>) : null}
                              </>)}
                          </Link>
...
function TopBar({ title, subtitle, actions }) {
    ...
    return (<header className="sticky top-0 z-30 mb-6 glass-panel rounded-[28px] px-4 py-4 sm:px-6">
              ...
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="h-12 rounded-2xl border-border/60 bg-card/45 px-3">
                  ...
                  <div className="hidden text-left sm:block">
                    <p className="text-sm font-medium text-foreground">{accountSummary.adminName}</p>
                    <p className="text-xs text-muted-foreground">{accountSummary.adminRole}</p>
                  </div>
...
export function AppShell({ title, subtitle, actions, children }) {
    ...
    return (<div className={cn("min-h-screen bg-background text-foreground", theme === "dark" ? "dark" : "")}>
      <div className="app-shell-bg min-h-screen">
        <div className="pointer-events-none fixed inset-0 overflow-hidden">
          <div className="surface-dots absolute inset-0 opacity-20"/>
          <motion.div className="absolute left-[12%] top-24 h-40 w-40 rounded-full bg-primary/14 blur-3xl" animate={{ y: [0, 24, 0], x: [0, 16, 0] }} transition={{ duration: 12, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut" }}/>
          <motion.div className="absolute bottom-20 right-[8%] h-52 w-52 rounded-full bg-secondary/12 blur-3xl" animate={{ y: [0, -18, 0], x: [0, -12, 0] }} transition={{ duration: 14, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut" }}/>
        </div>

        <div className="relative mx-auto flex min-h-screen max-w-[1600px] gap-6 p-4 lg:p-6">
          <AnimatePresence initial={false}>
            <motion.aside layout className="hidden xl:block" animate={{ width: sidebarCollapsed ? 112 : 320 }} transition={{ type: "spring", stiffness: 240, damping: 28 }}>
              <SidebarContent />
            </motion.aside>
          </AnimatePresence>
          ...
          <main className="flex min-w-0 flex-1 flex-col">
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: "easeOut" }} className="flex-1">
              {children}
            </motion.div>
          </main>
        </div>
